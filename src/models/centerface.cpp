...
int Centerface::detect(ncnn::Mat &inblob, std::vector<FaceInfo> &faces, int resized_w, int resized_h, float scoreThresh,
                       float nmsThresh) {
    if (inblob.empty()) {
    ...
    while (vScores.size() > 0) {
        ...
        for (std::multimap<float, int>::iterator it = vScores.begin(); it != vScores.end();) {
            ...
            if (type == NMS_UNION)
                ...
                IOU = IOU /
                      ((input.at(it_idx).area < input.at(last).area) ? input.at(it_idx).area : input.at(last).area);
            }
            if (IOU > nmsthreshold) {
                it = vScores.erase(it);
            } else {
...
void Centerface::decode(ncnn::Mat &heatmap, ncnn::Mat &scale, ncnn::Mat &offset, ncnn::Mat &landmarks,
